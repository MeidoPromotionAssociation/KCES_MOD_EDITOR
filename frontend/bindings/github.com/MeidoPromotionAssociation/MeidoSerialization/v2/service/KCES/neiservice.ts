// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

/**
 * NeiService 专门处理 KCES 使用的 .nei 共用格式 / NeiService handles the shared .nei format used by KCES
 * @module
 */

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import { Call as $Call, CancellablePromise as $CancellablePromise, Create as $Create } from "@wailsio/runtime";

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as KCES$0 from "../../serialization/KCES/models.js";
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as nei$0 from "../../serialization/common/nei/models.js";

/**
 * ConvertCSVToNei 将 CSV 转换为 .nei 文件
 * ConvertCSVToNei converts CSV to a .nei file
 */
export function ConvertCSVToNei(inputPath: string, outputPath: string): $CancellablePromise<void> {
    return $Call.ByID(4044986106, inputPath, outputPath);
}

/**
 * ConvertNeiToCSV 将 .nei 文件转换为 CSV
 * ConvertNeiToCSV converts a .nei file to CSV
 */
export function ConvertNeiToCSV(inputPath: string, outputPath: string): $CancellablePromise<void> {
    return $Call.ByID(3127832606, inputPath, outputPath);
}

/**
 * ReadNeiFile 读取并解码 .nei 文件
 * ReadNeiFile reads and decodes a .nei file
 */
export function ReadNeiFile(path: string): $CancellablePromise<KCES$0.Nei | null> {
    return $Call.ByID(1749658000, path).then(($result: any) => {
        return $$createType1($result);
    });
}

/**
 * WriteNeiFile 编码并写入 .nei 文件
 * WriteNeiFile encodes and writes a .nei file
 */
export function WriteNeiFile(path: string, value: KCES$0.Nei | null): $CancellablePromise<void> {
    return $Call.ByID(2623042313, path, value);
}

// Private type creation functions
const $$createType0 = nei$0.Table.createFrom;
const $$createType1 = $Create.Nullable($$createType0);
